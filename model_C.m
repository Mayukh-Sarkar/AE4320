...
for i = 1 : 1
    ...
    for m = 1 : 1
        ...
        for k = 1 : length(omegaf)
            H_pe(k) = Mag(k) ;
            f = @(z) sum(abs(H_pe(k) - z(1)*(1 + z(2)*(1j*omegaf(k))) /(1+z(3)*(1j*omegaf(k)))* exp(-1j*omegaf(k)*z(4)) * (z(6)^2/(z(6)^2 + 2*z(5)*z(6)*1j*omegaf(k) + (1j*omegaf(k))^2)))^2);
            data_magC(i,k) = Mag(k) ;
        ...
        [z,fvalC] = fminsearch(f,z0,options);
        %z0 = [0,0,0,0,0,0];
        lb = [0, 0, 0, 0,0, 5];
